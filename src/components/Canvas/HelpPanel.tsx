  ...
  {open&&<div className="help-overlay" role="dialog" aria-modal="true" aria-labelledby="help-title" onClick={event=>{event.stopPropagation();if(event.target===event.currentTarget)setHelpOpen(false)}} onPointerDown={event=>event.stopPropagation()}>
   <div className="help-modal" onClick={event=>event.stopPropagation()} onPointerDown={event=>event.stopPropagation()}>
    <div className="help-header">
     ...
     <button ref={closeRef} type="button" className="help-close" aria-label="Close help" onClick={event=>{event.stopPropagation();setHelpOpen(false)}} onPointerDown={event=>event.stopPropagation()}><X size={16} weight="regular" aria-hidden="true" /></button>
    </div>
    <div className="help-content">
     <section className="help-section">
      ...
      <ul>
       ...
       <li><code>Enter</code> saves Â· <code>Shift + Enter</code> adds a new line Â· <code>Esc</code> cancels Â· clicking away saves.</li>
       <li><code>+</code> on a node adds a child. The trash icon deletes (branches ask for confirmation).</li>
       <li>Drag the six-dot grip handle to reposition a node. This never changes the hierarchy.</li>
     ...
     <section className="help-section">
      ...
      <ul>
       <li>Click the chevron <code>â€º</code> to collapse or expand one branch only.</li>
     ...
     <section className="help-section">
      ...
      <ul>
       <li>Click the status dot to cycle: none â†’ Failed (red) â†’ Review (amber) â†’ Mastered (green).</li>
       <li>Red = couldn&apos;t recall Â· Amber = partially remembered Â· Green = solid.</li>
      </ul>
     </section>
     <section className="help-section">
      <h3>Shortcuts</h3>
     ...
     <section className="help-section">
      ...
      <ul>
       <li>The heatmap panel shows your page-wide stats. Minimize it to a slim bar or hide it with its â€“ and Ã— buttons.</li>
     ...
     <section className="help-section">
      ...
      <ul>
       ...
       <li>View bar: <code>âˆ’</code> zoom out Â· <code>%</code> reset Â· <code>+</code> zoom in Â· magnifier icon fits everything on screen.</li>
