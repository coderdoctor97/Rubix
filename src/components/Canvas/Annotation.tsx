...
export default function Annotation({ann}:{ann:AnnotationType}) {
 ...
 return <div className={`annotation ann-${ann.kind}${hover?' is-hover':''}${editing?' is-editing':''}`} style={{left:ann.position.x+(dragOff?.x??0),top:ann.position.y+(dragOff?.y??0)}} onPointerDown={onDown} onPointerMove={onMove} onPointerUp={onUp} onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}>
  ...
  {editing
   ? <textarea ref={input} className="ann-editor" placeholder="Add a note…" spellCheck={false} value={draft} onChange={e=>setDraft(e.target.value)} onBlur={save} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();e.stopPropagation();save()}else if(e.key==='Escape'){e.preventDefault();e.stopPropagation();cancel()}}} onInput={adjust}/>
   : <div className={`ann-content${ann.content?'':' is-empty'}`} onClick={()=>setEditing(true)}>{ann.content||'Add a note…'}</div>}
 </div>;
}
