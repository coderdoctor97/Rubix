'use client';
import {useEffect,useRef,useState,useMemo,type PointerEvent, memo} from 'react';
import type {Node as NodeType, NodeSize, NodeStyle, TiptapDoc} from '@/lib/types';
...
import {formatPresentationOrder} from '@/lib/operations/presentation';
...
function NodeInner({node, childIds, viewportZoom}:{node:NodeType;childIds:string[];viewportZoom:number}) {
  ...
  const selectForInteraction=useCanvasStore(s=>s.selectForInteraction);
  const connectingFrom=useCanvasStore(s=>s.connectingFrom);
  const setConnectingFrom=useCanvasStore(s=>s.setConnectingFrom);
  const setMouseWorld=useCanvasStore(s=>s.setMouseWorld);
  const magneticTarget=useCanvasStore(s=>s.magneticTarget);
  const setMagneticTarget=useCanvasStore(s=>s.setMagneticTarget);
  const createConnection=useCanvasStore(s=>s.createConnection);
  const update=useCanvasStore(s=>s.update);
  const setEditing=useCanvasStore(s=>s.setEditing);
  const createChild=useCanvasStore(s=>s.createChild);
  const createIndependentTopic=useCanvasStore(s=>s.createIndependentTopic);
  const duplicateNode=useCanvasStore(s=>s.duplicateNode);
  const remove=useCanvasStore(s=>s.remove);
  const setNodeTint=useCanvasStore(s=>s.setNodeTint);
  const setNodeStyle=useCanvasStore(s=>s.setNodeStyle);
  const moveNodes=useCanvasStore(s=>s.moveNodes);
  const moveNodesLive=useCanvasStore(s=>s.moveNodesLive);
  ...
  const [preview,setPreview]=useState<NodeSize|null>(null);
  const size=preview??node.size;
  const gripStart=useRef<{x:number;y:number;w:number;h:number}|null>(null);
  ...
  const dragStart=useRef<{sx:number;sy:number;origins:Record<string,{x:number;y:number}>}|null>(null);
  const isDraggingNode=useRef(false);

  useEffect(()=>{ if(!paletteOpen) return; const onKey=(e:KeyboardEvent)=>{ if(e.key==='Escape') setPaletteOpen(false); }; window.addEventListener('keydown', onKey); return ()=> window.removeEventListener('keydown', onKey); },[paletteOpen]);
  ...
  useEffect(()=>{ if(!styleOpen) return; const onKey=(e:KeyboardEvent)=>{ if(e.key==='Escape') setStyleOpen(false); }; window.addEventListener('keydown', onKey); return ()=> window.removeEventListener('keydown', onKey); },[styleOpen]);

  useEffect(()=>{
    if(!menuOpen&&!paletteOpen&&!styleOpen)return;
    const onDown=(e:globalThis.PointerEvent)=>{
      const t=e.target as HTMLElement|null;
      if(cardRef.current&&t&&!cardRef.current.contains(t)){
        setMenuOpen(false);setPaletteOpen(false);setStyleOpen(false);
      }
    };
    document.addEventListener('pointerdown',onDown);
    return()=>document.removeEventListener('pointerdown',onDown);
  },[menuOpen,paletteOpen,styleOpen]);

  const commitRich=(rtDoc:TiptapDoc,rtPlain:string)=>{
    setEditing(null);
    const plan=planRichCommit(node.content,node.doc,rtDoc,rtPlain);
    if(plan.kind==='noop')return;
    update(c=>{c.nodes[node.id].doc=plan.doc;c.nodes[node.id].content=plan.content;c.nodes[node.id].updatedAt=Date.now()})
  };
  ...
  const onGripDown=(e:PointerEvent<HTMLButtonElement>)=>{
    ...
    const card=(e.currentTarget as HTMLElement).closest('.node-card') as HTMLElement|null;
    gripStart.current={x:e.clientX,y:e.clientY,w:node.size?.width??NODE_WIDTH,h:node.size?.height??(card?.offsetHeight||NODE_MIN_HEIGHT)};
    (e.currentTarget as HTMLElement).setPointerCapture(e.pointerId)
  };
  const onGripMove=(e:PointerEvent<HTMLButtonElement>)=>{
    const g=gripStart.current;if(!g)return;
    const w=Math.max(160,g.w+(e.clientX-g.x)),h=Math.max(NODE_MIN_HEIGHT,g.h+(e.clientY-g.y));
    setPreview({width:w,height:h});setNodeSize(node.id,{width:w,height:h})
  };
  const onGripUp=(e:PointerEvent<HTMLButtonElement>)=>{
    const g=gripStart.current;if(!g)return;gripStart.current=null;
    const w=Math.max(160,g.w+(e.clientX-g.x)),h=Math.max(NODE_MIN_HEIGHT,g.h+(e.clientY-g.y));
    setPreview(null);if(w!==g.w||h!==g.h)update(c=>{c.nodes[node.id].size={width:w,height:h};c.nodes[node.id].updatedAt=Date.now()})
  };
  const onGripDblClick=(e:React.MouseEvent)=>{
    e.stopPropagation();e.preventDefault();
    if(node.size)update(c=>{c.nodes[node.id].size=null;c.nodes[node.id].updatedAt=Date.now()})
  };
  ...
  const renderFormatted=(text:string)=>{
    ...
    return spans.map((s,i)=>{
      ...
      if(s.italic)n=<em key={i}>{n}</em>;
      if(s.bold)n=<strong key={i}>{n}</strong>;
      if(s.underline)n=<u key={i}>{n}</u>;
      return n;
    })
  };
  ...
  const onNodeDragEnd=(e:PointerEvent)=>{
    ...
    if(isDraggingNode.current){
      const dx=(e.clientX-ds.sx)/viewportZoom,dy=(e.clientY-ds.sy)/viewportZoom;
      moveNodes(Object.entries(ds.origins).map(([id,orig])=>({id,position:{x:orig.x+dx,y:orig.y+dy}})));
    }
    isDraggingNode.current=false;
  };
  ...
  return (
    <div
      ...
      onPointerLeave={()=>{if(window.matchMedia('(hover: hover)').matches)setHoverId(null)}}
      onPointerDown={onNodeDragStart}
    ...
    >
      ...
      <button
        ...
        onPointerDown={e=>{ e.stopPropagation(); e.preventDefault(); setConnectingFrom(node.id); }}
        onPointerMove={e=>{
        ...
        onPointerUp={e=>{
          if(connectingFrom&&connectingFrom!==node.id){ createConnection(connectingFrom,node.id); }
