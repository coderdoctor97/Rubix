...
import {PaletteIcon, Plus, DotsThree, Trash, CaretRight} from '@phosphor-icons/react';
...
function NodeInner({node, childIds, viewportZoom}:{node:NodeType;childIds:string[];viewportZoom:number}) {
  ...
  // Outside-click dismissal: close all menus when pointerdown lands outside the card.
  // Replaces the broken position:fixed backdrops (which fail inside #world's transform).
  useEffect(()=>{
  ...
  const commitRich=(rtDoc:TiptapDoc,rtPlain:string)=>{setEditing(null);const plan=planRichCommit(node.content,node.doc,rtDoc,rtPlain);if(plan.kind==='noop')return;update(c=>{c.nodes[node.id].doc=plan.doc;c.nodes[node.id].content=plan.content;c.nodes[node.id].updatedAt=Date.now()})};
  ...
  return <div ref={cardRef} className={`node-card ${roleClass} ${styleClass} ${editing===node.id?'is-editing':''} ${just===node.id?'node-enter':''} ${node.tint?'is-tinted':''} ${isSelected?'is-selected':''} ${size?'is-sized':''} ${isRevealing?'node-reveal':''} ${magneticTarget===node.id?'node-magnet-target':''}`} style={cardStyle} onPointerDownCapture={e=>{if(e.button===0&&!isSelected)selectForInteraction(node.id)}} onPointerEnter={e=>{if(window.matchMedia('(hover: hover)').matches)setHoverId(node.id)}} onPointerLeave={()=>{if(window.matchMedia('(hover: hover)').matches)setHoverId(null)}} onPointerDown={onNodeDragStart} onPointerMove={onNodeDragMove} onPointerUp={onNodeDragEnd}>
    ...
    <div className="node-top"><div className="node-main"><StatusBadge status={node.status} onClick={cycle}/>{childIds.length>0&&<button className={`chevron ${node.isCollapsed?'':'open'}`} onClick={()=>toggleNode(node.id)} aria-label={node.isCollapsed?'Expand':'Collapse'}><CaretRight size={16} weight="regular" aria-hidden="true" /></button>}{editor}</div></div>
