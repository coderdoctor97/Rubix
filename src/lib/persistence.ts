...
export function loadUISettings():{heatmapMode:HeatmapMode;theme:ThemeId} { try { const raw=localStorage.getItem(UI_SETTINGS_KEY); if(!raw)return {heatmapMode:'mini',theme:'light'}; const data=JSON.parse(raw); if(!data||typeof data!=='object') return {heatmapMode:'mini',theme:'light'}; return {heatmapMode:HEATMAP_MODES.includes(data.heatmapMode)?data.heatmapMode as HeatmapMode:'mini',theme:typeof data.theme==='string'&&data.theme?data.theme:'light'}; } catch { return {heatmapMode:'mini',theme:'light'}; } }
