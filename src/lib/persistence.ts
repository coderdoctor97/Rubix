import type { Annotation, CanvasData, CanvasIndex, Connection, CustomTheme, HeatmapMode, ThemeId } from './types';
...
export function migrateCanvas(raw: unknown): CanvasData | null {
  try {
    ...
    // Connections: optional; tolerate missing or garbage by keeping only valid entries
    let connections: Connection[] | undefined;
    if (Array.isArray(data.connections)) {
      const valid = (data.connections as unknown[]).filter((c): c is Connection => {
        if (!c || typeof c !== 'object' || Array.isArray(c)) return false;
        const cn = c as Record<string, unknown>;
        return (
          typeof cn.a === 'string' && cn.a.length > 0 &&
          typeof cn.b === 'string' && cn.b.length > 0
        );
      });
      // Deduplicate by sorted pair (undirected)
      const seen = new Set<string>();
      const deduped: Connection[] = [];
      for (const cn of valid) {
        const key = [cn.a, cn.b].sort().join('\0');
        if (!seen.has(key)) { seen.add(key); deduped.push(cn); }
      }
    ...
    if (Array.isArray(data.annotations)) {
      const valid = (data.annotations as unknown[]).filter((a): a is Annotation => {
        ...
        return (
          typeof ann.id === 'string' && ann.id.length > 0 &&
      ...
      if (valid.length > 0) result.annotations = valid;
    }
    return result;
  } catch {
    return null;
...
function createSeedCanvas(id: string): CanvasData {
  const t = Date.now();
  let seq = 0;
  const c: CanvasData = { id, name: 'My Canvas', nodes: {}, viewport: { x: 0, y: 0, zoom: 1 }, connections: [], createdAt: t, updatedAt: t };
...
export function loadOrCreateCanvas(id: string): CanvasData {
  ...
  const now = Date.now();
  return { id, name: 'Untitled', nodes: {}, viewport: { x: 0, y: 0, zoom: 1 }, connections: [], createdAt: now, updatedAt: now };
}
...
export type UISettings={heatmapMode:HeatmapMode;theme:ThemeId;specialMode:boolean};
export function loadUISettings():UISettings { try { const raw=localStorage.getItem(UI_SETTINGS_KEY); if(!raw)return {heatmapMode:'mini',theme:'light',specialMode:false}; const data=JSON.parse(raw); if(!data||typeof data!=='object') return {heatmapMode:'mini',theme:'light',specialMode:false}; return {heatmapMode:HEATMAP_MODES.includes(data.heatmapMode)?data.heatmapMode as HeatmapMode:'mini',theme:typeof data.theme==='string'&&data.theme?data.theme:'light',specialMode:data.specialMode===true}; } catch { return {heatmapMode:'mini',theme:'light',specialMode:false}; } }
export function saveUISettings(settings:UISettings):void { try { localStorage.setItem(UI_SETTINGS_KEY,JSON.stringify(settings)); } catch { /* ignore quota / privacy-mode errors */ } }
