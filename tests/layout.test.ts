...
describe('tidyPositions', () => {
  ...
  it('re-places descendants in createdAt order using cascade rules', () => {
    ...
    expect(c1?.position).toEqual({ x: HORIZONTAL_INDENT, y: NODE_MIN_HEIGHT + VERTICAL_GAP });
    expect(c2?.position).toEqual({ x: HORIZONTAL_INDENT, y: (NODE_MIN_HEIGHT + VERTICAL_GAP) * 2 });
    expect(g1?.position).toEqual({ x: HORIZONTAL_INDENT * 2, y: (NODE_MIN_HEIGHT + VERTICAL_GAP) * 2 });
  });
  ...
  it('returns no moves when already tidy', () => {
    const nodes: Record<string, Node> = {
      root: makeNode('root', null, 0, 0),
      c1: makeNode('c1', 'root', HORIZONTAL_INDENT, NODE_MIN_HEIGHT + VERTICAL_GAP, 1),
    };
